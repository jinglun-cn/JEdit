...
public class SearchDialog extends EnhancedDialog
{
	...
	private void initFocusOrder() 
	{
		...
		focusOrder.add(filterModifiedDate);
	} //}}}
	...
	private JCheckBox filterModifiedDate;
	...
	private JPanel createMultiFilePanel()
	{
		...
		modifiedFrom = new HistoryTextField("search.modifiedFrom");
		modifiedFrom.addActionListener(actionListener);
		cons.insets = new Insets(0,0,3,0);
		label = new JLabel(jEdit.getProperty("search.modifiedFromField"),
		...
		modifiedTo = new HistoryTextField("search.modifiedTo");
		modifiedTo.addActionListener(actionListener);
		cons.insets = new Insets(0,0,3,0);
		label = new JLabel(jEdit.getProperty("search.modifiedToField"),
				SwingConstants.RIGHT);
		label.setBorder(new EmptyBorder(0,0,0,12));
		...
		label = new JLabel(jEdit.getProperty("search.filterField"),
			SwingConstants.RIGHT);
		label.setBorder(new EmptyBorder(0,0,0,12));
		...
		filterModifiedDate = new JCheckBox(jEdit.getProperty("search.filterModifiedDate"));
		//filterModifiedDate.setSelected(jEdit.getBooleanProperty("search.filterModifiedDate.toggle", true));
		dirCheckBoxPanel.add(filterModifiedDate);
	...
	private void updateEnabled()
	{
		...
		filterModifiedDate.setEnabled(searchDirs);
		...
		try
		{
			...
			else if(searchDirectory.isSelected())
			{
				...
				if(fileset instanceof DirectoryListSet)
				{
					...
					if (filterModifiedDate.isSelected()){
                        dset.setFilterModifiedDate(true, modifiedFrom.getText(), modifiedTo.getText());
                        modifiedFrom.addCurrentToHistory();
						modifiedTo.addCurrentToHistory();
					} else
						dset.setFilterModifiedDate(false, modifiedFrom.getText(), modifiedTo.getText());
				}
				else if (filterModifiedDate.isSelected()) {
					fileset = new DirectoryListSet(directory, filter, recurse,
							modifiedFrom.getText(), modifiedTo.getText());
					modifiedFrom.addCurrentToHistory();
					modifiedTo.addCurrentToHistory();
				}
				else
	...
	private void load()
	{
		...
		SearchFileSet fileset = SearchAndReplace.getSearchFileSet();
		HistoryModel model;
        // set filterModifiedDate history model here
		model = modifiedFrom.getModel();
		if(model.getSize() != 0)
			modifiedFrom.setText(model.getItem(0));
		else
		{
			modifiedFrom.setText("2020/02/05");
		}
		model = modifiedTo.getModel();
		if(model.getSize() != 0)
			modifiedTo.setText(model.getItem(0));
		else
		{
			modifiedTo.setText("2020/05/05");
		}
		...
		if(fileset instanceof DirectoryListSet)
		{
			modifiedFrom.setText(((DirectoryListSet)fileset)
					.getModifiedFrom());
			modifiedTo.setText(((DirectoryListSet)fileset)
					.getModifiedTo());
			filter.setText(((DirectoryListSet)fileset)
				.getFileFilter());
			directory.setText(((DirectoryListSet)fileset)
